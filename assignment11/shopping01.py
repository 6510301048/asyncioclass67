...
async def checkout_customer(queue: Queue, cashier_number: int):
    while not queue.empty():
        ...
        print(f"The cashier_{cashier_number}"
              f"finished chechout customer_{customer.customer_id}"
...
async def main():
    ...
    customer_start_time = time.perf_counter()
    customer_product = asyncio.create_task(customer_generation(customer_queue, 10))
    cashiers = [checkout_customer(customer_queue, i) for i in range(4)]

    await asyncio.gather(customer_product, *cashiers)
    print(f"The supermaket process finished"
          f"{customer_product.result()} customers"
            f"in{round(time.perf_counter() - customer_start_time, ndigits=2)} secs")   
